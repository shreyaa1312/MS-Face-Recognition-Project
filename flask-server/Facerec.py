...
def facerec(home):
    ...
    # if face found, checks in known-persons-facelist to check if the person
    # is a known person or stranger
    for face in response_detection:
        try:
            known_faces = face_client.face_list.get("known-persons-facelist")
            similar_faces = face_client.face.find_similar(
                face.face_id, "known-persons-facelist")
        except:
            pass
    ...
    # if the face not found in known-persons-facelist, adds the person visit date
